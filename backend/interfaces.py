...
Retrieval routes through the structured retrieval engine, with explicit degraded
lexical fallback using the SQL knowledge repository. JSONL memory is export-only.
...
def retrieve(query: str, k: int) -> list[dict]:
    ...
    """
    from backend.rag.retrieve import retrieve as _impl
    return _impl(query, k)


def remember(record: dict) -> None:
    """Persist a resolved incident record into the memory store."""
    # Legacy debug/export only; this does not create trusted retrieval evidence.
    from backend.rag.store import remember as _impl
    _impl(record)
...
def emit_event(event: dict) -> None:
    ...
    """
    from backend.tools.event_bus import emit_event as _impl
    _impl(event)
